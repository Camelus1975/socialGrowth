...
`;
    ...
    if (contentWriter && contentWriter.result && contentWriter.result.copy_variants) {
      ...
      try {
        ...
        if (generatedImages.some(url => url)) {
          steps.push({ agent: "Creative Director", log: "Saving generated images to Media Assets Manager..." });
          ...
          steps.push({ agent: "Creative Director", log: `${generatedImages.filter(u => u).length} images saved to Media Assets ✓` });
        }
    ...
    try {
      ...
      if (operationsToInsert.length > 0) {
        ...
        steps.push({ agent: "System", log: `Created ${operationsToInsert.length} pending agent operations for CEO approval.` });
      }
    } catch (opErr) {
