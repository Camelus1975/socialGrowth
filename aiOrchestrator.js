...
`;
    ...
    // Process Organic Content (images + text posts)
    ...
    if (contentWriter && contentWriter.result && contentWriter.result.copy_variants) {
      ...
      try {
          ...
          for (let i = 0; i < generatedImages.length; i++) {
            ...
            try {
              ...
              if (authHeader && authHeader.includes('mock-supabase-jwt-token')) {
                console.log(`[Orchestrator] Mock token detected. Skipping DB insert for media image ${i+1}.`);
              } else {
                await fetch(`${config.SUPABASE_URL}/rest/v1/media`, {
                  ...
                  body: JSON.stringify({
                    app_id: appId,
                    name: `${appName || 'Campaign'} - ${platform} Visual`,
        ...
        // Build organic text+image posts
        contentWriter.result.copy_variants.forEach((v, i) => {
          const date = new Date();
        ...
        console.error("Failed to process organic posts", err);
      }
    }

    // ======== VIDEO POST (runs independently of ContentWriter) ========
    ...
    if (videoAgent && videoAgent.result && videoAgent.result.video_concept) {
      try {
        ...
        if (config.REPLICATE_API_TOKEN) {
          try {
            ...
            if (prediction.error) {
              ...
              let pollCount = 0;
              while ((status === 'starting' || status === 'processing') && pollCount < 36) {
                pollCount++;
                ...
                console.log(`[Orchestrator] Video status: ${status}... (poll ${pollCount})`);
              }
        ...
        console.error("[Orchestrator] Video post assembly error:", videoErr);
      }
    }

    // ======== DB INSERT (runs once for ALL posts — organic + video) ========
    if (postsToInsert.length > 0 && userId) {
      try {
        if (authHeader && authHeader.includes('mock-supabase-jwt-token')) {
          console.log('[Orchestrator] Mock token detected. Skipping DB insert for scheduled posts.');
        } else {
          const insertRes = await fetch(`${config.SUPABASE_URL}/rest/v1/scheduled_posts?select=*`, {
            method: 'POST',
            headers: {
              'apikey': config.SUPABASE_ANON_KEY,
              'Authorization': authHeader,
          ...
          if (!insertRes.ok) {
            ...
            console.error('[Orchestrator] Insert error:', errText);
          }
        }
        await pushLog("Publishing Agent", `${postsToInsert.length} posts successfully scheduled in the Calendar.`);
      } catch (err) {
        console.error("Failed to insert posts into calendar", err);
      }
    }
    ...
    try {
      if (config.SUPABASE_URL && appId) {
        if (authHeader && authHeader.includes('mock-supabase-jwt-token')) {
          console.log('[Orchestrator] Mock token detected. Skipping agent_operations error logging.');
        } else {
          await fetch(`${config.SUPABASE_URL}/rest/v1/agent_operations`, {
            method: 'POST',
            headers: {
              'apikey': config.SUPABASE_ANON_KEY,
              'Authorization': authHeader,
