          ...
          for (let i = 0; i < generatedImages.length; i++) {
            ...
            try {
              ...
              if (authHeader && authHeader.includes('mock-supabase-jwt-token')) {
                ...
                const mediaRes = await fetch(`${config.SUPABASE_URL}/rest/v1/media`, {
                  method: 'POST',
                  headers: {
                ...
                if (!mediaRes.ok) console.error(`[Orchestrator] Failed to save image ${i+1} to media:`, await mediaRes.text());
              }
            } catch (mediaErr) {
